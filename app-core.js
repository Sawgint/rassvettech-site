// app-core.js — маршруты, лайтбокс, почта, hash, без CSS-инъекции (используем rassvet.css)
(function () {
  "use strict";
  ...
  // ---------------- CONTACT EMAILS (НЕ кликабельная версия) ----------------
  // Поддержка обоих span-ов (contactEmail1, contactEmail2) + бэкап
  function applyContactEmails() {
    const el1 = $("#rassvet-root #contactEmail1");
    const el2 = $("#rassvet-root #contactEmail2");

    const saved1 = (localStorage.getItem("rassvettech_email_1") || "").trim();
    const saved2 = (localStorage.getItem("rassvettech_email_2") || "").trim();

    const fallback1 = el1 ? (el1.textContent || "").trim() : "Varzanov.86@mail.ru";
    const fallback2 = el2 ? (el2.textContent || "").trim() : "varzanov0602@gmail.com";

    const v1 = saved1 || fallback1;
    const v2 = saved2 || fallback2;

    if (el1) el1.textContent = v1;
  ...
  window.RASSVETTECH_APP = {
    ...
    setEmails(email1, email2) {
      if (email1) localStorage.setItem("rassvettech_email_1", String(email1));
      if (email2) localStorage.setItem("rassvettech_email_2", String(email2));
      applyContactEmails();
    },
...
// app-i18n.js — полные строки локализаций EN / RU / JP / KO / ZH (полная версия)
(function () {
  "use strict";
  const $  = (s, r=document) => r.querySelector(s);
  const $$ = (s, r=document) => Array.from(r.querySelectorAll(s));
  ...
  const STR = {
    ...
    jp: {
      ...
      mission_text:
        "人々を尊重する優しい有用なAIデバイスを作ります。静かさを設計に、倫理を標準に、そして毎日家庭や診療所、学びの場で役立つほどシンプルに。夜明けはすべての人のものです。",
      ...
      tattoo_use_2: "医療センター（タトゥー/瘢痕/表識の除去）。",
    ...
    ko: {
      ...
      mission_text:
        "우리는 사람을 존중하는 부드럽고 유용한 AI 기기를 만듭니다. 설계상 조용하고, 기본적으로 윤리적이며, 매일 가정·병원·학습에서 도움이 될 만큼 단순합니다. 새벽은 모두의 것입니다.",
      nav_drone: "ANDrew 드론",
      nav_mask: "EIDOS 마스크",
      nav_robot: "AnestAI",
      nav_mirror: "VITA 미러",
      ...
      tattoo_feats_4: "AI 제어: 피부/색소를 분석해 최적 파라미터 설정.",
      tattoo_use_h: "활용",
    ...
    zh: {
      ...
      mission_title: "我们的使命",
      mission_text:
        "我们打造温和有用、尊重人的AI设备：安静设计，默认伦理，简单到每天都能在家庭、诊所和学习中发挥作用。黎明属于所有人。",
